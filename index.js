...
global.db = new Low(
  /https?:\/\//.test(opts['db'] || '') ?
    new cloudDBAdapter(opts['db']) : /mongodb/.test(opts['db']) ?
      new mongoDB(opts['db']) :
      new JSONFile('./database/user/data.json')
)
...
function konsol() {
      console.clear()
      console.log(chalk.bold.cyan(figlet.textSync('WA BOT - CGPT', {
         font: 'Small',
...
${chalk.greenBright('Owner')}: ${chalk.whiteBright(`${config.ownerName}`)}
...
const connectToWhatsapp = async () => {
	const danzz = WAConnect({
		...
		browser: ["Danzz Bot","Chrome","2.0.0"],
		auth: state
	})
konsol()
store.bind(danzz.ev)
...
danzz.ev.on('groups.update', async come => {
	try {
    for(let wel of come) {
       try {
       ppgc = await danzz.profilePictureUrl(wel.id, 'image')
       } catch {
       ppgc = 'https://tinyurl.com/yx93l6da'
       }
       let pp_group = { url : ppgc }
       if (wel.announce == true) {
       danzz.send5ButImg(wel.id, `「 Group Settings Change 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message`, pp_group, [])
       } else if (wel.announce == false) {
       danzz.send5ButImg(wel.id, `「 Group Settings Change 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan !`, `Group Settings Change Message`, pp_group, [])
       } else if (wel.restrict == true) {
       danzz.send5ButImg(wel.id, `「 Group Settings Change 」\n\nInfo group telah dibatasi, Sekarang hanya admin yang dapat mengedit info group !`, `Group Settings Change Message`, pp_group, [])
       } else if (wel.restrict == false) {
       danzz.send5ButImg(wel.id, `「 Group Settings Change 」\n\nInfo group telah dibuka, Sekarang peserta dapat mengedit info group !`, `Group Settings Change Message`, pp_group, [])
       } else {
       danzz.send5ButImg(wel.id, `「 Group Settings Change 」\n\nGroup Subject telah diganti menjadi *${wel.subject}*`, `Group Settings Change Message`, pp_group, [])
    	}
    }
    } catch (err){
    console.log(err)
